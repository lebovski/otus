...
template <typename T> struct WaitingQueue {
  WaitingQueue()
   ...
   *  @note This function will wake-up only one thread waiting on the pop
   * function.
   *  @note Template TT only to use perfect forwarding here.
   */
  template <typename TT> void push(TT &&value) {
    // Create new node
...
private:
  ...
  /** @brief Takes entry from the head of the WaitingQueue with waiting in case
   * of empty.
   *  @param entry result entry
   *  @note This function is thread-safe.
   *  @note This is blocking function!
...
void example() {
  ...
  std::thread work_producer_thread{work_producer, std::ref(work_queue),
                                   std::ref(stop_flag)};
