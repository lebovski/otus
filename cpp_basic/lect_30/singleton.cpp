...
struct Logger {
public:
  Logger(const char *filename) : m_file{filename} {
    if (!m_file.is_open()) {
  ...
  void info(const std::string &msg) { m_file << "[INFO] " << msg << std::endl; }
...
struct Some_class {
public:
  Some_class(Logger &logger) : m_logger{logger} {}

  void do_work() { m_logger.debug("Some_class - do_work"); }

private:
  Logger &m_logger;
};

void some_function(Logger &logger) { logger.error("some_function - call"); }
...
private:
  explicit Logger(const char *path) : m_file{path} {
    if (!m_file.is_open()) {
